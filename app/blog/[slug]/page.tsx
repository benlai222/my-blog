...
export default function BlogPost({ params }: { params: { slug: string } }) {
    ...
    return (
        <main className="pt-10 pb-24 px-4 sm:px-6 lg:px-8 relative min-h-screen">
            {/* JSON-LD Schema */}
            <Script
                id="article-schema"
            ...
            {/* Background wrap with overflow-hidden to prevent blur from scaling page */}
            <div className="absolute top-0 right-0 bottom-0 left-0 overflow-hidden pointer-events-none -z-20">
                <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-brand-pink/15 blur-[120px] rounded-full pointer-events-none"></div>
            </div>

            <div className="max-w-7xl mx-auto flex flex-col xl:flex-row gap-8 xl:gap-12 relative z-10 justify-center">
                <article className="relative w-full max-w-3xl flex-1 glass-panel rounded-[2.5rem] p-8 sm:p-12 lg:p-16 mt-8 border-brand-pink/30 shadow-[0_10px_50px_rgba(0,0,0,0.5)]">
                    {/* 麵包屑/返回控制項 */}
                    <nav className="mb-12">
                        <Link
                            href="/"
                ...
                {/* 側邊導覽目錄 */}
                <aside className="hidden xl:block w-64 flex-shrink-0 mt-8 relative">
                    <TableOfContents />
                </aside>
            </div>
        </main>
